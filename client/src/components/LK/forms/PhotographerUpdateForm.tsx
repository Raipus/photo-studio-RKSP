...
interface PhotographerModalProps {
	isOpen: boolean
	onClose: () => void
	onSubmit: (data: IPhotographerCreate) => Promise<void>
	register: UseFormRegister<IPhotographerCreate>
	errors: FieldErrors<IPhotographerCreate>
	handleSubmit: UseFormHandleSubmit<IPhotographerCreate, undefined>
}

const PhotographerUpdateForm: React.FC<PhotographerModalProps> = ({
	isOpen,
	onClose,
	onSubmit,
	register,
	...
	return (
		...
		>
			...
			>
				<h2 className='mb-4 text-xl font-semibold'>Изменить фотографа</h2>
				<button
				...
				<form onSubmit={handleSubmit(onSubmit)}>
					...
					<label className='mb-2 block'>
						Временный новый пароль:
						<p className='mb-2 block text-sm'>(Необязательно)</p>
						<input
							type='password'
							placeholder='Пароль'
							className='mt-1 block w-full rounded-md border p-2 text-black'
...
export default PhotographerUpdateForm
